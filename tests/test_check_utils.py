...
class CheckUtilsTests(unittest.TestCase):
    ...
    def test_check_is_positive(self):
        self.assertIsNone(check_utils.check_is_positive("number", 1.1))

        with self.assertRaises(ValueError):
            check_utils.check_is_positive("number", -5)

    def test_check_is_positive_int(self):
        self.assertIsNone(check_utils.check_is_positive_int("integer", 5))

        with self.assertRaises(TypeError):
            check_utils.check_is_positive_int("integer", "string")

        with self.assertRaises(ValueError):
            check_utils.check_is_positive_int("integer", -10)
